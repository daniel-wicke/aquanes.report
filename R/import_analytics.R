...
fill_datetime <- function(df,
  ...
  columns_raw_data <- grep(names(df),
...
import_sheet <- function(xlsPath,
  ...
  tmp_par1 <- readxl::read_excel(path = xlsPath,
                                 sheet = sheet,
                                 col_names = col_names,
                                 skip = skip)
  ...
  tmp_par2 <- fill_datetime(tmp_par1[,cols_with_headers],
  ...
  tmp_par3 <- set_timezone(tmp_par2,
                           tz = tz_org)

  ### Define time zone be be used for export
  tmp_par4 <- change_timezone(tmp_par3,
                              tz = tz_export)
  ...
  tmp_par5_list <- tidyr::gather_(data = tmp_par5,
                                  key_col = "Keys",
                                  value_col = "ParameterValue",
                                  gather_cols = col_values )

  tmp_par6_list <- tidyr::separate_(tmp_par5_list,
                                    col = "Keys",
...
import_sheets <- function(xlsPath,
  ...
  sites <- readxl::read_excel(path = xlsPath,
                              sheet = sheet_sites)

  location <- readxl::read_excel(path = xlsPath,
                              sheet = sheet_location)

  parameters <- readxl::read_excel(xlsPath,
  ...
  for (sheet_index in seq_along(sheets_analytics)) {
    ...
    if (dbg) {
      print(sprintf("Importing & normalising analytics sheet: '%s' from '%s'",
                           mySheet,
                           basename(xlsPath)))
    }

    tmp <- import_sheet(xlsPath = xlsPath,
                        sheet = mySheet,
                        col_rawData_pattern = col_rawData_pattern,
...
plot_analytics <- function(df) {
  ...
  for (loc_index in seq_along(locIDs)) {
    ...
    title_label <- sprintf("Location: %s (ID: %s)",
    ...
    grDevices::pdf(file = file.path(pdfDir, sprintf("%d_analytics_timeSeries.pdf", locIDs[loc_index])),
        width = 10,
        height = 7)
    for (i in seq_len(n_pages)) {
      g1 <- ggplot2::ggplot(tmp, aes_string(x = "DateTime",
                                            y = "ParameterValue",
                                            col = "SiteLabel")) +
        ggforce::facet_wrap_paginate(~ ParameterName,
                                     nrow = 1,
                                     ncol = 1,
                                     scales = "free_y",
                                     page = i) +
    ...
    grDevices::pdf(file = file.path(pdfDir, sprintf("%d_analytics_allSites_onePlot.pdf", locIDs[loc_index])),
        width = 10,
        height = 7)
    for (i in seq_len(n_pages)) {
      g2 <- ggplot2::ggplot(tmp, aes_string(x = "SiteLabel",
                                            y = "ParameterValue",
                                            col = "SiteLabel")) +
        ggforce::facet_wrap_paginate(~ ParameterName,
                                     nrow = 1,
